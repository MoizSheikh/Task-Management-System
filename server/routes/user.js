...
router.post("/signup",(req,res)=>{
    ...
    if(newuser.password!=newuser.password2)return res.status(201).json({message: "Password not match",success:false});

    User.findOne({email: req.body.email},(err,user)=>{
        if(user) return res.status(201).json({ auth : false, message :"Email Already Exits",success:false});

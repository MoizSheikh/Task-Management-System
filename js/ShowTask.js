...
function getData(){
    ...
    fetch('task/getAll/'+id).then((response)=>{
        return response.json();
    }).then((data)=>{
        console.log(data);
        
        fetch('task/assign/get/'+id).then((response1)=>{
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        start_date.innerHTML=`<span>Start Date: </span>${new Date(element.start_date).toDateString()}`;
        end_date.innerHTML=`<span>End Date:</span> ${new Date(element.end_date).toDateString()}`
    ...
    for (let i = 0; i < assignedTasks.length; i++) {
        ...
        if(element.end_date){
            end_time.innerHTML=`<span>Start Time: ${new Date(element.end_date).toLocaleTimeString()}</span>`
        ...
        <div><span>Assigned to: </span>${element.user_id.username}</div><span>Status: ${element.status}</span>`;
        ...
        removeBtn.onclick=function(e){
            let id=e.target.id;
            fetch(`task/assign/${id}`,{method:'DELETE'}).then((response2)=>{
                return response2.json();
            }).then((data2)=>{
                if(data2.success){
    ...
    for (let i = 0; i < completedTasks.length; i++) {
        ...
        <div> Completed By ${element.user_id.username}</div><span>Status: ${element.status}</span>
        `;

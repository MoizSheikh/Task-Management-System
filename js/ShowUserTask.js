    ...
    .then((data) => {
      ...
      for (let i = 0; i < todoData.length; i++) {
        ...
        startBtn.onclick = function (event) {
          ...
          fetch(`/task/assign/start_date/${btn_id}`, options)
        ...
        completeBtn.onclick = function (event) {
          ...
          fetch(`/task/assign/complete/${element._id}`, options)
            .then((response) => response.json())
            .then((data) => {
              console.log(data);
              if (data.success) {
      ...
      for (let i = 0; i < completedTasks.length; i++) {
        ...
        }</div><div><span>Assigned By: ${element.admin_id.username} - ${
          element.admin_id.email
        }</span></div>
        `;
